...
public class JwtAutorisationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String autorization = request.getHeader(JwtConstant.AUTORIZATION);
        String token;
        String usernameFromToken = null;
        DecodedJWT decodedJWT= null;
        if (autorization != null && autorization.startsWith(JwtConstant.BEARER)) {
            JWTVerifier verifier = JWT.require(Algorithm.HMAC512(JwtConstant.SECRET_KEY)).build();
            token = autorization.substring(JwtConstant.BEARER.length());
            decodedJWT = verifier.verify(token);
            usernameFromToken = decodedJWT.getSubject();
        }
