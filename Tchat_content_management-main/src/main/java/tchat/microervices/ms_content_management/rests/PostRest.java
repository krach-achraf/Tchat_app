...
public class PostRest {
    ...
    @GetMapping("/{username}/{page}/{size}")
    public List<PostResponseDTO> findPostsByUser(@PathVariable String username, @PathVariable int page, @PathVariable int size) {
        log.info("PostRest: Retrieve posts with page " + page + " and size " + size);
        return postService.findPostsByUser(username, page, size);
    }
